...
.ad-editor{position:relative;display:inline-block;cursor:grab;border:3px dashed #ff6600;background:rgba(255,102,0,0.04);user-select:none;min-width:100px;min-height:30px}
.ad-editor .ad-editor-img{max-width:100%;height:auto;max-height:none}
.ad-editor-handle{position:absolute;width:14px;height:14px;background:#fff;border:3px solid #ff6600;z-index:40;cursor:pointer}
.ad-handle-nw{top:-8px;left:-8px;cursor:nwse-resize}
.ad-handle-ne{top:-8px;right:-8px;cursor:nesw-resize}
.ad-handle-sw{bottom:-8px;left:-8px;cursor:nesw-resize}
.ad-handle-se{bottom:-8px;right:-8px;cursor:nwse-resize}
.ad-editor.ad-dragging{opacity:0.8;z-index:99999}
.ad-editor.ad-resizing{opacity:0.8;z-index:99999}
...
function editAd(el) {
    var pos = el.getAttribute('data-ad-pos') || el.dataset.adPos;
    if (!pos) return;
    var url = prompt('أدخل رابط صورة الإعلان:', el.getAttribute('data-ad-img') || '');
    if (url === null) return;
    var link = prompt('أدخل رابط النقر (اختياري):', el.getAttribute('data-ad-link') || '');
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'admin/ajax-save-ad.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.onload = function() { if (xhr.status === 200) location.reload(); };
    xhr.send('position=' + encodeURIComponent(pos) + '&image=' + encodeURIComponent(url || '') + '&link=' + encodeURIComponent(link || ''));
}

// Ad drag & resize
(function(){
    var dragging = null, dragOffX = 0, dragOffY = 0, origOx = 0, origOy = 0;
    var resizing = null, resizeStartX = 0, resizeStartY = 0, resizeStartW = 0, resizeStartH = 0, resizeStartL = 0, resizeStartT = 0, resizeStartCX = 0, resizeStartCY = 0, resizeCorner = '';

    document.addEventListener('mousedown', function(e){
        var rh = e.target.closest('.ad-editor-handle');
        if (rh) {
            var ed = rh.closest('.ad-editor');
            if (ed) {
                e.preventDefault();
                resizing = ed;
                resizeCorner = Array.from(rh.classList).find(function(c){ return c.match(/ad-handle-/); });
                resizeStartX = e.clientX;
                resizeStartY = e.clientY;
                resizeStartW = ed.offsetWidth;
                resizeStartH = ed.offsetHeight;
                resizeStartL = parseFloat(ed.style.left) || 0;
                resizeStartT = parseFloat(ed.style.top) || 0;
                resizeStartCX = resizeStartL + ed.offsetWidth / 2;
                resizeStartCY = resizeStartT + ed.offsetHeight / 2;
                ed.classList.add('ad-resizing');
            }
            return;
        }
        var ed = e.target.closest('.ad-editor');
        if (ed && !e.target.closest('.ad-editor-handle')) {
            e.preventDefault();
            dragging = ed;
            origOx = parseFloat(ed.style.left) || 0;
            origOy = parseFloat(ed.style.top) || 0;
            dragOffX = e.clientX;
            dragOffY = e.clientY;
            ed.classList.add('ad-dragging');
        }
    });

    document.addEventListener('mousemove', function(e){
        if (dragging) {
            var dx = e.clientX - dragOffX;
            var dy = e.clientY - dragOffY;
            dragging.style.left = (origOx + dx) + 'px';
            dragging.style.top = (origOy + dy) + 'px';
        }
        if (resizing) {
            var dx = e.clientX - resizeStartX;
            var dy = e.clientY - resizeStartY;
            var newW, newH;
            if (resizeCorner.indexOf('nw') >= 0) {
                newW = resizeStartW - dx; newH = resizeStartH - dy;
            } else if (resizeCorner.indexOf('ne') >= 0) {
                newW = resizeStartW + dx; newH = resizeStartH - dy;
            } else if (resizeCorner.indexOf('sw') >= 0) {
                newW = resizeStartW - dx; newH = resizeStartH + dy;
            } else {
                newW = resizeStartW + dx; newH = resizeStartH + dy;
            }
            if (e.shiftKey) {
                ...
                if (Math.abs(newW - resizeStartW) > Math.abs(newH - resizeStartH)) {
                    newH = newW / ratio;
                } else {
                    newW = newH * ratio;
                }
            ...
            // Centered resize: keep center fixed
            resizing.style.left = (resizeStartCX - newW / 2) + 'px';
            resizing.style.top = (resizeStartCY - newH / 2) + 'px';
    ...
    document.addEventListener('mouseup', function(e){
        if (dragging) {
            dragging.classList.remove('ad-dragging');
            var ox = parseFloat(dragging.style.left) || 0;
            var oy = parseFloat(dragging.style.top) || 0;
            if (Math.abs(ox) > 2000) { ox = 0; }
            if (Math.abs(oy) > 2000) { oy = 0; }
            dragging.style.left = ox + 'px';
            dragging.style.top = oy + 'px';
            saveAdOffset(dragging.getAttribute('data-ad-pos'), ox, oy);
            dragging = null;
        }
        if (resizing) {
            resizing.classList.remove('ad-resizing');
            saveAdSize(resizing);
            resizing = null;
        }
    });

    function saveAdSize(el) {
        var pos = el.getAttribute('data-ad-pos');
        if (!pos) return;
        var w = el.offsetWidth, h = el.offsetHeight;
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'admin/ajax-save-ad.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.send('position=' + encodeURIComponent(pos) + '&width=' + w + '&height=' + h);
    }

    function saveAdOffset(pos, ox, oy) {
        // Clamp extreme offsets
        if (Math.abs(ox) > 500) ox = 0;
        if (Math.abs(oy) > 500) oy = 0;
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'admin/ajax-save-ad.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.send('position=' + encodeURIComponent(pos) + '&offset_x=' + ox + '&offset_y=' + oy);
    }



    // Restore saved offset on load
    (function restoreAdOffset() {
        var eds = document.querySelectorAll('.ad-editor');
        eds.forEach(function(ed){
            var ox = parseFloat(ed.getAttribute('data-ad-ox')) || 0;
            var oy = parseFloat(ed.getAttribute('data-ad-oy')) || 0;
            if (Math.abs(ox) > 2000) ox = 0;
            if (Math.abs(oy) > 2000) oy = 0;
            if (ox || oy) {
                ed.style.left = ox + 'px';
...
function render_ad($position) {
    ...
    $clickUrl = 'ad-request.php?pos=' . urlencode($position);
    if (!$ad) {
        if ($isAdmin) {
            echo '<div class="ad-banner" data-ad-pos="' . htmlspecialchars($position) . '" data-ad-img="" data-ad-link="" style="cursor:pointer;" onclick="editAd(this)">انقر لإضافة إعلان</div>';
        } else {
            echo '<a href="' . $clickUrl . '" class="ad-banner" style="display:block">إعلان | مساحة إعلانية متاحة</a>';
        }
        return;
    }
    if ($ad['type'] === 'image') {
        $img = htmlspecialchars($ad['image']);
        $link = htmlspecialchars($ad['link'] ?? '');
        if ($isAdmin) {
            $w = $ad['width'] ?? '';
            $h = $ad['height'] ?? '';
            $ox = $ad['offset_x'] ?? 0;
            $oy = $ad['offset_y'] ?? 0;
            $padStyle = '';
            if ($w) $padStyle .= 'width:' . $w . 'px;';
            if ($h) $padStyle .= 'height:' . $h . 'px;';
            if ($ox || $oy) $padStyle .= 'left:' . $ox . 'px;top:' . $oy . 'px;';
            echo '<div class="ad-editor" data-ad-pos="' . htmlspecialchars($position) . '" data-ad-img="' . $img . '" data-ad-link="' . $link . '" data-ad-w="' . $w . '" data-ad-h="' . $h . '" data-ad-ox="' . $ox . '" data-ad-oy="' . $oy . '" style="' . $padStyle . '">';
            if ($img) {
                echo '<img src="' . $img . '" alt="إعلان" class="ad-banner-img ad-editor-img" style="display:block;max-width:100%;height:auto;' . ($ox ? 'transform:translate(' . $ox . 'px,' . $oy . 'px);' : '') . '">';
            } else {
                echo '<div class="ad-banner" style="margin:0;border:none;background:#fafafa;min-height:50px">ضع صورة للإعلان</div>';
            }
            echo '<span class="ad-editor-handle ad-handle-nw"></span>';
            echo '<span class="ad-editor-handle ad-handle-ne"></span>';
            echo '<span class="ad-editor-handle ad-handle-sw"></span>';
            echo '<span class="ad-editor-handle ad-handle-se"></span>';
            echo '</div>';
        } else {
            echo '<a href="' . $clickUrl . '" style="display:inline-block;max-width:100%"><img src="' . $img . '" alt="إعلان" class="ad-banner-img"></a>';
        }
    } elseif ($ad['type'] === 'code') {
        if ($isAdmin) {
            echo '<div data-ad-pos="' . htmlspecialchars($position) . '" style="cursor:pointer;" onclick="editAd(this)"><a href="' . $clickUrl . '">' . $ad['code'] . '</a></div>';
        } else {
            echo '<a href="' . $clickUrl . '">' . $ad['code'] . '</a>';
        }
    } else {
        if ($isAdmin) {
            echo '<div class="ad-banner" data-ad-pos="' . htmlspecialchars($position) . '" data-ad-img="" data-ad-link="" style="cursor:pointer;" onclick="editAd(this)">انقر لإضافة إعلان</div>';
